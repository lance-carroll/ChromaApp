...
-- Campaign membership lives here instead of on character sheets. This keeps
-- GM-owned campaign state separate from player-owned character state.
...
using (auth.uid() = owner_id)
with check (auth.uid() = owner_id and source_type <> 'system');
...
using (auth.uid() = owner_id or public.can_read_character_as_campaign_gm(character_id));
...
with check (
  auth.uid() = owner_id
  and (public.is_character_owner(character_id) or public.can_read_character_as_campaign_gm(character_id))
);
...
using (auth.uid() = owner_id or public.can_read_character_as_campaign_gm(character_id))
with check (auth.uid() = owner_id or public.can_read_character_as_campaign_gm(character_id));
